# %% [markdown]
# Codice per la simulazione della rete stradale del quartiere Bicocca e calcolo dei tempi di percorrenza di percorsi interi alla rete stradale.
...
# %%
def vel(roads:list, t:int, x:int, index :int) -> float:
    global V_max
    return (1-roads[t][index][x])*V_max

def tempo_percorrenza(index:list, roads:list, it:int)-> float:
    global xx,tt,dx
    T = 0
    pos = 0
    N_cella = 0
    N_istante = it
    for i in index:
        while N_cella < len(xx[i]) and N_istante < len(tt)-1:
            pos_pr = pos
            pos += vel(roads,N_istante,N_cella,i)*(tt[N_istante+1]-T-tt[it])
            if pos >= dx[i]*(N_cella+1):
                pos = dx[i]*(N_cella +1)
                T += (pos-pos_pr)/vel(roads,N_istante,N_cella,i)
                N_cella +=1
            else:
                T = tt[N_istante +1]-tt[it]
                N_istante += 1
        pos = 0
        N_cella = 0
    return T
...
for k in range(len(tt)-1):
  ...
  temp = scheme(temp, vectGodunov, dt/dx_min, colore_S1, colore_S2)
  for i in range(11):
    temp.append([E[i,k]])
U.append(temp)
dump(temp,f)
...
# %%
percorsi = []
percorsi.append([46,47,42,6,12,13,35,18,25,23])
percorsi.append([46,47,42,6,33,10,29,27,25,23])
percorsi.append([22,24,26,40,15,11,9,7,44])
percorsi.append([21,20,17,14,11,9,7,44])

# %%
tempi_percorsi = np.zeros([len(percorsi),len(time_stamps)])
for k in range(len(percorsi)):
    for i in range(len(time_stamps)):
        tempi_percorsi[k][i] = tempo_percorrenza(percorsi[k],U,int(time_stamps[i]))

# %%
ist_partenza = np.zeros(len(time_stamps))
for i in range(len(ist_partenza)):
    ist_partenza[i] = tt[int(time_stamps[i])]

# %%
list_labels = []
for i in range(len(time_stamps)):
    list_labels.append('{:.0f}'.format(i))

# %%
max_percorso = np.amax(tempi_percorsi)

# %%
fig, ax = plt.subplots(4, figsize = (20,20))
plt.tight_layout(pad = 3.6)
for i in range(len(percorsi)):
    ax[i].plot(time_stamps,tempi_percorsi[i], marker = 'o')
    ax[i].set_xlabel('Istante di partenza [min]')
    ax[i].set_ylabel('Tempo di percorrenza [s]')
    ax[i].set_xticks(time_stamps)
    ax[i].set_xticklabels(list_labels)
    ax[i].set_ylim(0,max_percorso+50)
    
ax[0].set_title("Percorso $r_5-r_6-r_1-I_7-I_{13}-I_{14}-j_4-I_{19}-I_{26}-I_{24}$")
ax[1].set_title("Percorso $r_5-r_6-r_1-I_7-j_2-I_{11}-I_{30}-I_{28}-I_{26}-I_{24}$")
ax[2].set_title("Percorso $I_{23}-I_{25}-I_{27}-j_9-I_{16}-I_{12}-I_{10}-I_8-r_3$")
ax[3].set_title("Percorso $I_{22}-I_{21}-I_{18}-I_{15}-I_{12}-I_{10}-I_8-r_3$")

# %%
f = open('tempi50', 'wb')
dump(time_stamps,f)
dump(percorsi,f)
dump(tempi_percorsi,f)
dump(list_labels,f)
dump(ist_partenza,f)
dump(ax,f)
